...
# --- SYSTEM DESIGN SYSTEM (RGB STRIP, UNIQUE NEON BUTTONS & FULL-WIDTH FIX) ---
st.markdown("""
    <style>
    ...
    html, body, .stApp { 
        background: linear-gradient(rgba(11, 9, 26, 0.93), rgba(6, 4, 15, 0.97)), 
    ...
    /* Force Blocks to Take Absolute 100% Mobile Screen Width Without Margins */
    [data-testid="stVerticalBlock"], .element-container, div[data-testid="stVerticalBlockBorderWrapper"] {
        width: 100% !important;
    ...
    /* Input Form Modern Blocks stretching full width */
    div[data-testid="stVerticalBlockBorderWrapper"] {
        background-color: rgba(22, 19, 41, 0.90) !important;
...
if not st.session_state.logged_in:
    ...
    st.markdown("<p style='text-align:center; color:#00ffcc; font-size:15px; font-weight:800; margin-bottom:20px;'>🔑 AUTHORIZATION ACCESS LINK PROMPTS</p>", unsafe_allow_html=True)
    ...
    with st.container():
        if st.session_state.auth_mode == "Login":
            ...
            if st.button("Create Brand New Account"):
                st.session_state.auth_mode = "Register"
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
            
            st.markdown('<div class="btn-violet">', unsafe_allow_html=True)
            if st.button("Forgot Security Password Lock?"):
...
# --- APPLICATION DESKTOP INTERFACE RENDER LAYER ---
    ...
    if st.session_state.is_admin:
        ...
        if st.button("🔗 Modify Redirect Task URL Targets"): switch_panel("Edit Task Redirects")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        if st.session_state.selected_panel == "Pending Requests":
            ...
            st.markdown('<div class="btn-pink">', unsafe_allow_html=True)
            if st.button("Deploy New Target Redirect Path Structure"):
    ...
    # --- CLIENT INTERFACE VIEWS ---
        ...
        <div class="metric-card-box">
            ...
            <h1 style="font-size:42px; font-weight:900; color:#00ffcc; margin:5px 0;">RM {wallet_bal:,.2f}</h1>
