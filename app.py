...
st.markdown("""
    ...
    *, p, label, span, button, div { 
    ...
    div.stButton > button p, div.stFormSubmitButton > button p { color: white !important; }
...
# Strict sidebar visibility handling
...
# --- AUTH PANELS PIPELINE (TOTAL ISOLATION) ---
if not st.session_state.logged_in:
    ...
    if st.session_state.verification_stage == "awaiting_otp":
        ...
        with st.form("forgot_form"):
            ...
            c_send, c_back = st.columns(2)
            with c_send:
                send_req = st.form_submit_button("Send Token")
            with c_back:
                back_login = st.form_submit_button("Back to Login")
            ...
            if send_req:
                ...
                if user_match:
                    ...
                    st.rerun()
                else: st.error("Target email layout node not found.")
            if back_login:
        ...
        with st.form("reset_finalize_form"):
            ...
            if st.form_submit_button("Overwrite Credentials", use_container_width=True):
        ...
        with st.form("reg_form"):
            ...
            with c_log:
                goto_log = st.form_submit_button("Back to Login")
        ...
        with st.form("login_form"):
            ...
            # Master Action Controls encapsulated securely INSIDE the form block
            submit_login = st.form_submit_button("Authorize Secure Access")
            ...
            with c2:
                goto_forgot = st.form_submit_button("🔑 Forgot Pass?")
            ...
            if submit_login:
                if login_email.strip() == "admin" and login_pass.strip() == "admin123":
                    ...
                    if user_record and user_record[0] == login_pass.strip():
                        ...
                        st.rerun()
                    else: st.error("Credentials pairing failed database matching.")
...
# --- MAIN LOGGED-IN PORTAL INTERFACE WORKSPACE (STRICT SEPARATION) ---
else:
    if st.session_state.is_admin:
        ...
        # --- PREMIUM SIDEBAR WORKSPACE NAVIGATION HUB ---
        with st.sidebar:
            st.markdown("### 🧭 NAVIGATION MENU")
            app_tab = st.radio(
                "Select Workspace Action Panel:",
                ["Dashboard Overview", "Stream Video Tasks", "Add Wallet Funds", "Bank Cashout Liquidation", "Ledger Logs Statements"],
                label_visibility="collapsed"
            )
            st.markdown("---")
            if st.button("🚪 Disconnect Secure Portal", use_container_width=True):
                st.session_state.logged_in = False
                st.session_state.auth_view = "login"
                st.rerun()
