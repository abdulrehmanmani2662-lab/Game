...
# --- BRANDING & STYLING OVERHAUL (1000047431_3.jpg Match) ---
st.markdown("""
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    
    <style>
    /* Clean layout, removing native junk */
    ...
    * { font-family: 'Plus Jakarta Sans', sans-serif !important; }
    ...
    /* PREMIUM DASHBOARD WIDGET CARDS (Image 3 Style) */
    .earnwise-main-card {
    ...
    .card-sub-banner { font-size: 12px !important; color: #fbbf24 !important; font-weight: 700; margin-top: 2px; }
    ...
    /* Action quick grid elements */
    .action-btn-container { display: flex !important; gap: 12px !important; margin-top: 16px !important; }
    .action-badge-btn {
        flex: 1 !important; text-align: center !important; padding: 12px !important; 
        border-radius: 8px !important; font-size: 13px !important; font-weight: 700 !important; color: white !important;
    }
    .bg-blue { background: #1d4ed8 !important; }
    .bg-green { background: #047857 !important; }
...
if not st.session_state.logged_in:
    ...
    # 2. Forgot Password Gateway
    elif st.session_state.auth_view == "forgot_password_request":
        st.markdown('<div class="clean-auth-card">', unsafe_allow_html=True)
        with st.form("forgot_form"):
            st.markdown("<h3>Reset Password</h3><p style='color:#64748b; font-size:13px;'>Enter registered email to receive OTP token code.</p>", unsafe_allow_html=True)
            ...
            if st.form_submit_button("Send Recovery Token Code", use_container_width=True):
                ...
                if user_match:
                    ...
                    st.rerun()
                else: st.error("Email not found in system storage logs.")
        st.markdown('</div>', unsafe_allow_html=True)
        if st.button("Back to Sign In Dashboard"):
            st.session_state.auth_view = "login"
            st.rerun()

    # 3. Forgot Password Processing OTP Verification
        ...
        with st.form("reset_finalize_form"):
            ...
            new_pass = st.text_input("New Password String", type="password")
            if st.form_submit_button("Overwrite Password", use_container_width=True):
                if input_token.strip() == st.session_state.generated_otp:
                    ...
                    st.success("Password recovery override successful! Login now.")
                    st.session_state.auth_view = "login"
                    st.rerun()
                else: st.error("Invalid token verification code.")
        st.markdown('</div>', unsafe_allow_html=True)

    # 4. Sign Up System Panel
    ...
    # 5. Native Login Panel Gateway
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("Secure Sign In Engine", use_container_width=True):
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Create New Profile"): st.session_state.auth_view = "signup"; st.rerun()
        with c2:
            if st.button("🔑 Forgot Passwords?"): st.session_state.auth_view = "forgot_password_request"; st.rerun()

# --- APPLICATION WORKSPACE NAVIGATION PANELS (Image 3 Architecture) ---
else:
    # Sidebar Navigation System Layout Engine
    with st.sidebar:
        st.markdown("<h3 style='color:#1e293b; padding-bottom:10px;'>Global Matrix Navigation</h3>", unsafe_allow_html=True)
        if st.button("🏠 Dashboard Dashboard Panel", use_container_width=True): st.session_state.active_sidebar_tab = "Dashboard"
        if st.button("📺 View Stream Tasks", use_container_width=True): st.session_state.active_sidebar_tab = "Tasks"
        if st.button("➕ Fund Deposit Gateway", use_container_width=True): st.session_state.active_sidebar_tab = "Deposit"
        if st.button("📤 Outflow Cashout Terminal", use_container_width=True): st.session_state.active_sidebar_tab = "Withdrawal"
        if st.button("📜 Transaction History Logs", use_container_width=True): st.session_state.active_sidebar_tab = "History"
        st.markdown("---")
        if st.button("🚪 Termination Leave App Connection", use_container_width=True):
    ...
    if st.session_state.is_admin:
        ...
        # Fetching records matrix variables mapping
        ...
        # ACTIVE ROUTER MATRIX INTERFACES
        if st.session_state.active_sidebar_tab == "Dashboard":
            ...
            # Grid Layout Engine matching Screen Dashboard 3 Exactly
            ...
            if (c_time - claim_stamp) < 86400:
                ...
                st.error(f"Cooldown active loop sequence block. Time remaining: {rem//3600}h {(rem%3600)//60}m")
            else:
                if st.button("Claim Stream Settlement Award Node", use_container_width=True):
                    ...
                    st.success(f"Execution tracking completion bonus logged successfully: +RM {bonus:.2f}")
                    st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)

        elif st.session_state.active_sidebar_tab == "Deposit":
            st.markdown('<div class="premium-widget-box"><div class="widget-title-head">Submit Deposit Payment Slip Terminal</div>', unsafe_allow_html=True)
            with st.form("dep_form"):
                holder = st.text_input("Sender Verified Account Holder Full Title:")
                tx_str = st.text_input("Bank System Verification Transaction Code (Trx ID):")
                ...
                if st.form_submit_button("Hantar Slip Proof Processing", use_container_width=True):
                    if holder and tx_str:
                        ...
                        st.success("Log submission cached into administration checking modules pipeline.")
            st.markdown('</div>', unsafe_allow_html=True)
            ...
            with st.form("with_form"):
                ...
                if st.form_submit_button("Execute Request Connection", use_container_width=True):
                    if w_val <= bal:
                        ...
                        st.rerun()
                    else: st.error("Out of bounds allocation bounds balance deficit overflow.")
            st.markdown('</div>', unsafe_allow_html=True)
            ...
            if not all_deps: st.markdown("<p style='color:#64748b; text-align:center;'>No log matrix entities mapped yet.</p>", unsafe_allow_html=True)
            for dl in all_deps:
